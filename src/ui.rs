...
impl ChatUi {
    pub fn new(interactive: bool, header: String) -> Result<Self> {
        let plain = Ui::stdio();
        let fullscreen = interactive.then(|| FullScreen::new(header)).transpose()?;
        Ok(Self { plain, fullscreen })
    }
    ...
    pub fn set_header(&mut self, header: String) -> Result<()> {
        match self.fullscreen.as_mut() {
            Some(screen) => screen.set_header(header),
            None => {
                print!("{header}");
                io::stdout().flush()?;
                Ok(())
    ...
    pub fn expand_last(&mut self) -> Result<bool> {
        match self.fullscreen.as_mut() {
            Some(screen) => screen.set_last_collapsed(false),
            None => Ok(false),
        }
    }
    ...
    pub fn collapse_last(&mut self) -> Result<bool> {
        match self.fullscreen.as_mut() {
            Some(screen) => screen.set_last_collapsed(true),
            None => Ok(false),
        }
    }
    ...
    pub fn assistant_update(&mut self, raw_markdown: &str) -> Result<()> {
        match self.fullscreen.as_mut() {
            Some(screen) => screen.update_assistant(raw_markdown.to_string()),
            None => {
    ...
    pub fn notice(&mut self, text: &str) -> Result<()> {
        match self.fullscreen.as_mut() {
            Some(screen) => screen.add_notice(text),
            None => {
                println!("{text}");
                Ok(())
            }
    ...
    pub fn warning(&mut self, text: &str) -> Result<()> {
        match self.fullscreen.as_mut() {
            Some(screen) => screen.add_notice(format!("WARNING: {text}")),
            None => {
    ...
    pub fn error(&mut self, text: &str) -> Result<()> {
        match self.fullscreen.as_mut() {
            Some(screen) => screen.add_card(CardKind::Error, "Error", text),
            None => {
                eprintln!(
                    "{}",
                    self.plain
    ...
    fn card(
        ...
        match self.fullscreen.as_mut() {
            Some(screen) => screen.add_card(kind, title, body),
            None => {
...
fn render(frame: &mut Frame<'_>, model: &Model) {
    ...
    let footer_text = if model.prompt_visible {
        format!("{}\n{}", model.footer, "> ")
    } else {
        model.footer.clone()
    };
